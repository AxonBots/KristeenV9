...
from info import ADMINS, LOG_CHANNEL, USERNAME, DATABASE_NAME, FILES_DATABASE_NAME
...
async def get_ststs(bot, message):
    ...
    # Add database names to the stats output
    await message.reply_text(
        script.STATUS_TXT.format(
            users, groups, size, free, DATABASE_NAME,  # Add main DB name
            files, db2_size, db2_free, FILES_DATABASE_NAME,  # Add files DB name
            uptime, ram, cpu
        )
    )